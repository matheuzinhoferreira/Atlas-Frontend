async function alternarAtivo(id_usuario) {
...
function carregarUsuarios(tipoFiltro = 1) {
  ...
  async function carregarPagina(pagina = 1, tipo = tipoFiltro) {
    ...
    try {
      ...
      if (!resposta.ok) {
        ...
        throw new Error(`Erro na requisição: ${data['message']}`);
      }
  ...
  async function buscarDadosUsuarioDetalhado(idUsuario) {
    const token = localStorage.getItem("jwt-token-atlas");
    const url = `${window.apiBase.ip}/usuarios/info/${idUsuario}/${localStorage.getItem('tipo-usuario-atlas')}`;
    try {
      const resposta = await fetch(url, {
        method: "GET",
        headers: {
